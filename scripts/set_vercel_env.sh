...
for k in NEXT_PUBLIC_SUPABASE_URL NEXT_PUBLIC_SUPABASE_ANON_KEY NEXT_PUBLIC_B2_ENDPOINT NEXT_PUBLIC_B2_BUCKET; do
    add $k
done

# Set private environment variables for production  
for k in SUPABASE_SERVICE_ROLE_KEY B2_KEY_ID B2_APP_KEY; do
    add $k
done
